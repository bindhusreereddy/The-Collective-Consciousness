var hygein = './Dataset/reproductivepuzzle.json';
const fs = require('fs')
...
function runAnalysis(jsonString) {
    ...
    data.students.forEach(element => {
        ...
        getUsedWordsFrequency(element.usedWords,usedWords_freq);
        number_of_students++;
    });
...
function runData (returnType) {
    ...
    if(returnType == "wordsFrequency") return obj['usedWords_freq'];
}
...
function getUsedWordsFrequency(usedWords, usedWords_freq) {
    for (var i = 0; i < usedWords.length; i++) {
        usedWords_freq[usedWords[i]] = usedWords_freq[usedWords[i]] + 1;
    }
}

function getSingleUserData() {
    var readData = fs.readFileSync('./Dataset/sample_data_puzzle.json', 'utf8');
...
exports.getWordsFreq = () => runData("wordsFrequency");
