var hygein = './Dataset/stairsandslides.json';
const fs = require('fs')
...
function runAnalysis(jsonString) {
    ...
    data.students.forEach(element => {
        ...
        rolls.push(numberOfRolls(element));
...
function runData (returnType) {
    ...
    if(returnType == "rolls") return obj['rolls'];
}
...
function numberOfRolls(data){
    var id = data.id;
    var rolls = data.numberOfRolls;
    var obj = {
        'id': id,
        'rolls': rolls,
    };
...
function getSingleUserData() {
    var readData = fs.readFileSync('./Dataset/sample_data_snakeladder.json', 'utf8');
...
exports.getRolls = () => runData("rolls");
